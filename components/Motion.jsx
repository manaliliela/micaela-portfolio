...
const OFFSETS = {
  ...
  left: { x: -16 },
  right: { x: 16 },
  scale: { y: 8, scale: 0.98 },
  pop: { scale: 0.88, rotate: -3 },
};
...
export function Reveal({ children, delay = 0, y, from = "up", as = "div", className = "", style }) {
  ...
  return (
    <Tag
      ...
      animate={
        shown
          ? { opacity: 1, ...settle(off), transition: { duration: 0.55, delay, ease: EASE } }
...
// `light` = a quicker, gentler reveal (smaller movement, shorter fade) for busy grids.
const LIGHT_OFFSETS = {
  up: { y: 8 },
  down: { y: -6 },
  left: { x: -10 },
  right: { x: 10 },
  scale: { y: 5, scale: 0.99 },
  pop: { scale: 0.94 },
};

export function StaggerItem({ children, className = "", as = "div", from = "up", light = false, style, ...rest }) {
  const Tag = motion[as];
  const off = (light ? LIGHT_OFFSETS : OFFSETS)[from] || OFFSETS.up;
  const variants = {
    hidden: { opacity: 0, ...off, transition: INSTANT },
    show: { opacity: 1, ...settle(off), transition: { duration: light ? 0.38 : 0.5, ease: EASE } },
  };
  return (
  ...
  return (
    <span ref={ref} className={`maskLine ${className}`}>
      <motion.span
        className="maskInner"
        initial={{ y: "110%" }}
        animate={shown ? { y: "0%", transition: { duration: 0.55, delay, ease: EASE } } : { y: "110%", transition: INSTANT }}
      >
        {children}
      </motion.span>
    </span>
...
export function Float({ children, className = "", y = 10, rotate = 4, duration = 6, delay = 0, style }) {
  return (
    <motion.div
      className={className}
      style={style}
      animate={{ y: [0, -y * 0.6, 0], rotate: [-rotate * 0.6, rotate * 0.6, -rotate * 0.6] }}
...
export function Deco({
  ...
  return (
    <motion.div
      ...
      animate={
        shown
          ? { opacity, ...settle(off), transition: { duration: 0.7, delay, ease: EASE } }
          : { opacity: 0, ...off, transition: INSTANT }
      }
    >
      <motion.div
