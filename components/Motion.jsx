...
const OFFSETS = {
  ...
  left: { x: -24 },
  right: { x: 24 },
  scale: { y: 10, scale: 0.97 },
  pop: { scale: 0.8, rotate: -4 },
};
...
export function Reveal({ children, delay = 0, y, from = "up", as = "div", className = "", style }) {
  ...
  return (
    <Tag
      ...
      animate={
        shown
          ? { opacity: 1, ...settle(off), transition: { duration: 0.7, delay, ease: EASE } }
...
export function StaggerItem({ children, className = "", as = "div", from = "up", light = false, style, ...rest }) {
  ...
  const variants = {
    ...
    show: { opacity: 1, ...settle(off), transition: { duration: light ? 0.42 : 0.65, ease: EASE } },
  };
  return (
  ...
  return (
    <span ref={ref} className={`maskLine ${className}`}>
      <motion.span
        className="maskInner"
        initial={{ y: "110%" }}
        animate={shown ? { y: "0%", transition: { duration: 0.65, delay, ease: EASE } } : { y: "110%", transition: INSTANT }}
      >
        {children}
      </motion.span>
    </span>
...
export function Deco({
  ...
  return (
    <motion.div
      ...
      animate={
        shown
          ? { opacity, ...settle(off), transition: { duration: 0.8, delay, ease: EASE } }
          : { opacity: 0, ...off, transition: INSTANT }
      }
    >
      <motion.div
