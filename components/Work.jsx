...
function Card({ p, index, active, clone, onSelect }) {
  ...
  return (
    <StaggerItem light from={FROM[index % FROM.length]} className={`work work-${p.size} work-${p.slug} tone-${p.tone} ${active ? "isActive" : ""}`}>
      <motion.div
        className="workScale"
        initial={false}
...
export default function Work() {
  ...
  // Mouse: click and drag left/right like a swipe. Far enough = next/previous project;
  // otherwise it eases back. Touch keeps using the swipe handlers below.
  const onPointerDown = (e) => {
    if (e.pointerType !== "mouse" || e.button !== 0) return;
    const startX = e.clientX;
    const startPos = posRef.current;
    dragged.current = false;
    if (dragAnim.current) dragAnim.current.stop();
    dragX.set(0);
    ...
    const move = (ev) => {
      ...
      if (dragged.current) dragX.set(Math.max(-320, Math.min(320, dx)));
    };
    const up = (ev) => {
      ...
      const dx = ev.clientX - startX;
      if (dragged.current) {
        if (dx <= -70) go(startPos + 1);
        else if (dx >= 70) go(startPos - 1);
        dragAnim.current = animate(dragX, 0, { duration: 0.55, ease: EASE });
  ...
  return (
    <section className="section workSection" id="work" aria-labelledby="work-h">
      ...
      <div className="workViewport" ref={viewportRef} onKeyDown={onKeyDown} onPointerDown={onPointerDown} onDragStart={(e) => e.preventDefault()} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        <div
          className="workTrack"
          style={{
        ...
        >
          ...
          <Stagger className="workRow" stagger={0.03}>
            {[-1, 0, 1].flatMap((set) =>
              PROJECTS.map((p, i) => (
                <Card
                  key={`${set}-${p.slug}`}
                  ...
                  clone={set !== 0}
                  active={i === activeReal}
                  onSelect={() => { if (!dragged.current) go(set * count + i); }}
                />
              ))
            )}
          </Stagger>
