...
function Card({ p, index, active, onSelect }) {
  ...
  return (
    <StaggerItem from={FROM[index % FROM.length]} className={`work work-${p.size} work-${p.slug} tone-${p.tone} ${active ? "isActive" : ""}`}>
      <motion.div
        ...
        aria-label={active ? undefined : `Show ${p.client}`}
      >
        {featured ? mask : body}
      </motion.div>
    </StaggerItem>
  ...
  return (
    <section className="section workSection" id="work" aria-labelledby="work-h">
      ...
      <div className="workViewport" onKeyDown={onKeyDown} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        <div className="workTrack" style={{ transform: `translateX(calc(${index} * (-1 * (var(--cw) + var(--wg))) - var(--cw) / 2))` }}>
          <Stagger className="workRow" stagger={0.14}>
            {PROJECTS.map((p, i) => (
              <Card key={p.slug} p={p} index={i} active={i === index} onSelect={() => go(i)} />
            ))}
          </Stagger>
        </div>
      </div>
      <p className="srOnly" aria-live="polite">{`Showing project ${index + 1} of ${count}: ${PROJECTS[index].client}`}</p>
    </section>
  );
}
