...
import { Stagger, StaggerItem, Deco, EASE, useReplayInView } from "./Motion";
import { Sparkle, Flower, Blob, Loop, Dots, Heart } from "./Doodles";

export default function Process() {
  const [curveRef, curveShown] = useReplayInView("0px 0px -15% 0px");
  return (
    <section className="section" id="process" aria-labelledby="process-h">
      ...
      />
      <div className="processWrap" ref={curveRef}>
          ...
          <defs>
            <clipPath id="curveReveal">
              <motion.rect
                ...
                initial={{ width: 0 }}
                animate={curveShown ? { width: 1000, transition: { duration: 1.7, ease: EASE } } : { width: 0, transition: { duration: 0 } }}
              />
            </clipPath>
          </defs>
          <path
        ...
        <Stagger as="ol" className="steps" stagger={0.05}>
          {PROCESS.map((s, i) => (
            <StaggerItem as="li" key={s.n} light from={i % 2 ? "up" : "left"} className={`step stepPos${i}`}>
